...
public class Sorter <E> {
    public void selectionSort(List<E> data){
        for (int startingPlace = 0; startingPlace < data.size(); startingPlace ++){
            int indexOfSmallest = startingPlace;
            E smallestThing = data.get(startingPlace);
            for (int i = startingPlace; i < data.size(); i ++) {
                if (((Comparable) data.get(i)).compareTo(smallestThing) <= 0) {
                    indexOfSmallest = i;
                    smallestThing = data.get(i);
                }
            }
            E smallest = data.get(indexOfSmallest);
            data.set(indexOfSmallest, data.get(startingPlace));
            data.set(startingPlace, smallest);
        }
    }
